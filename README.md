...
- `.github/workflows/build-and-publish.yml` is a manually-triggered
  (`workflow_dispatch`) workflow: it checks whether breseq has a new commit
  since the last publish (`scripts/check_new_commit.sh`), and if so builds
  packages for Linux and macOS (Apple Silicon) and publishes them into the
  channel (`scripts/publish_channel.sh` + `conda index`) hosted on the
  `gh-pages` branch.
...
From the GitHub UI: Actions → "Build and publish breseq-prerelease" → Run
